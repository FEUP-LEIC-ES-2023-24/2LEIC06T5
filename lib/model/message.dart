...
Map<String, dynamic> findMostRecentMessage(
  ...
  for (Map<String, dynamic> message in userMessages) {
    ...
    if (mostRecentTimestamp == null ||
        messageTimestamp.compareTo(mostRecentTimestamp) > 0) {
  ...
  return mostRecentMessage ??
      {}; // Return an empty map if no messages are found
}
...
Future<List<Message>> getRecievedMessages(String userID) async {
  final snapshot = await db
      ...
      .where(Filter.and(
          Filter('recieverID',
              isEqualTo: "/user/${FirebaseAuth.instance.currentUser!.uid}"),
...
Future<List<Message>> getMostRecentMessagesOfUser(String userID) async {
  ...
  for (final user in users) {
    ...
    final snapshot = await db
        .collection('message')
        .where(Filter.or(
            Filter.and(Filter("senderID", isEqualTo: userID),
                Filter("recieverID", isEqualTo: user)),
            Filter.and(Filter("recieverID", isEqualTo: userID),
                Filter("senderID", isEqualTo: user))))
        .get();
